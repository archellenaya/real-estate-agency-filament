...
class NotionCommercialPropertySyncService extends NotionPropertySyncBaseService
{
    ...
    public function transform($data)
    {
        ...
        $marketType = '';
        $rentPeriod = null;
        switch (substr($property_ref, 0, 2)) {
            case 'CS':
                $marketType = 'Sale';
                break;
            case 'CL':
        ...
        $final_price = isset($data["Sell Asking Price"]["number"]) ? (float)$data["Sell Asking Price"]["number"] : null;
        $date_available = $data["Available on"]["date"]["start"] ?? null;
        $date_created = $data["Date Created"]["created_time"] ?? null;

        $propertyType = $data["Commercial Type"]["select"] ?? null;
        ...
        return [
            ...
            'old_id'                        => (string)($data["Commercial Address"]["title"][0]["plain_text"] ?? null),
